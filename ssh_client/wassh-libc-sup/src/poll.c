...
// Copy of wasi-libc's poll.c implementation until this fix is merged and we
// update to newer wasi-sdk release.
// https://github.com/WebAssembly/wasi-libc/pull/630
...
static int poll_wasip1(struct pollfd *fds, size_t nfds, int timeout) {
  ...
  __wasi_subscription_t subscriptions[maxevents];  // NOLINT(*)
  ...
  // Execute poll().
  size_t nevents;
  __wasi_event_t events[nsubscriptions];  // NOLINT(*)
    ...
    if (event->type == __WASI_EVENTTYPE_FD_READ ||
      ...
      if (event->error == __WASI_ERRNO_BADF) {
        ...
        if (event->type == __WASI_EVENTTYPE_FD_READ) {
            ...
            if (event->fd_readwrite.flags & __WASI_EVENTRWFLAGS_FD_READWRITE_HANGUP) {  // NOLINT(*)
              pollfd->revents |= POLLHUP;
            }
