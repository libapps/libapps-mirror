...
describe('terminal_settings_dropdown_tests.js', () => {
  ...
  beforeEach(function() {
    ...
    el = document.createElement('terminal-settings-dropdown');
  ...
  it('dropdown-updates-when-setting-changes', async function() {
    assert.equal(window.preferenceManager.get(preference), options[0]);
    assert.equal(el.shadowRoot.getElementById('select').value, options[0]);

    await window.preferenceManager.set(preference, options[1]);
    assert.equal(el.shadowRoot.getElementById('select').value, options[1]);

    await window.preferenceManager.set(preference, options[2]);
    assert.equal(el.shadowRoot.getElementById('select').value, options[2]);
  });

  it('setting-updates-when-dropdown-changes', async function() {
    assert.equal(window.preferenceManager.get(preference), options[0]);
    assert.equal(el.shadowRoot.getElementById('select').value, options[0]);

    el.uiValue_ = options[1];
    await lib.waitUntil(() => el.isConsistent());
    assert.equal(window.preferenceManager.get(preference), options[1]);

    el.uiValue_ = options[2];
    await lib.waitUntil(() => el.isConsistent());
