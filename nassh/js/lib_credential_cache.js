...
export class CredentialCache {
  constructor() {
    /**
     * The underlying cache. Keys are strings and the cache entries consist
     * of the encrypted data and the initialization vector (IV) used for the
    ...
    /**
     * The Web Crypto API AES-CBC CryptoKey that has been used to encrypt the
     * data in the cache. The key is randomly generated during initialization
    ...
    /**
     * Set to true if caching is enabled; false if caching is disabled and
     * null if the user has not yet made a decision.
     *
     * @private {?boolean}
     */
    this.enabled_ = null;
  ...
  async retrieve(key) {
    ...
    if (key in this.cache_) {
      ...
      // Remove cache entry to be added again only if data verification
      // succeeds.
