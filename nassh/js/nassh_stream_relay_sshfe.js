 ...
 */
export class RelaySshfeWsStream extends Stream {
  constructor() {
    super();
   ...
   * @return {!Promise<!AgentResponse>} A promise to resolve with the agent's
   *   response.
   */
   ...
   *
   * TODO: This uses gnubby-specific messages currently (113 & 114) to locate
   * the specific key to use to sign the challenge.
   *
   * @param {string} challenge The server challenge
   * @return {!Promise} A promise to resolve with the signed result.
   */
  signChallenge_(challenge) {
    // Construct a SSH_AGENTC_PUBLIC_KEY_CHALLENGE packet.
    //   byte    code
    //   byte    slot
    //   byte    alt
    // TODO: Rename "challenge" since it has nothing to do with |challenge|.
