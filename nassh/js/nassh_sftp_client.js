 ...
 *
 * @unrestricted For nasftp_cli.js to use Client[func].
 */
export class Client {
  /**
  ...
  /**
   * Parse the buffer and process the first valid packet in it,
   * else return false.
   *
   * @return {boolean}
   */
   ...
   * requesting packet's callback. If the response packet is valid and has
   * a valid request id, it will be returned with the callback. Finding and
   * executing a callback will return true, else will return false if an error
   * occurred.
   *
   * @param {!Packet} packet
   * @return {boolean}
   ...
   *    and requestId).
   * @return {!Promise<!Packet>} A Promise that resolves with the
   *     response packet.
   */
  sendRequest_(type, data) {
   ...
   * @return {!Packet}
   */
  isSuccessResponse_(responsePacket, requestType) {
    if (!(responsePacket instanceof StatusPacket)) {
          throw new TypeError('Received unexpected response to '
                              + requestType + ' packet: ' + responsePacket);
    }
   ...
   * @return {!Packet}
   */
  isNameResponse_(responsePacket, requestType) {
    if (responsePacket instanceof StatusPacket) {
   ...
   * @return {!Promise<!AttrsPacket>} A Promise that resolves with the
   *     remote file attributes, or rejects (usually with a StatusError).
   */
  setFileStatus(path, attrs) {
    const packet = new Packet();
    packet.setUtf8String(this.basePath_ + path);
   ...
   *     remote file attributes, or rejects (usually with a StatusError).
   */
  setFileHandleStatus(handle, attrs) {
    const packet = new Packet();
    packet.setString(handle);
    setFileAttrs(packet, attrs);
  ...
  removeDirectory(path, recursive = false) {
    // Low level directory remove packet.  Only works if the dir is already
    // empty.
    const rmdir = (path) => {
      const packet = new Packet();
      packet.setUtf8String(this.basePath_ + path);
   ...
   * @return {!Promise<!FileHandle>} A Promise that resolves with the
   *     remote file handle, or rejects (usually with a StatusError).
   */
  openFile(path, pflags) {
    const packet = new Packet();
    packet.setUtf8String(this.basePath_ + path);
   ...
   *
   * @param {string} handle The handle of the remote file.
   * @param {function(!ArrayBuffer)} callback The function called on every
   *     chunk.
   * @param {number=} offset The offset to start reading from.
   * @param {number=} length The maximum number of bytes to read.
   * @return {!Promise}
   */
  readChunks(handle, callback, offset = 0, length = undefined) {
   ...
   * @return {!Promise}
   */
  readFile(path, callback, offset = 0, length = undefined) {
    ...
    return this.sendRequest_('limits@openssh.com', packet)
      .then((response) => {
        return this.isExpectedResponse_(
            response, ExtendedReplyPacket, 'LIMITS');
      })
   ...
   * @return {!Promise<!NamePacket>} A Promise that resolves with the
   *     remote path, or rejects (usually with a StatusError).
   */
  realPath(path) {
    const packet = new Packet();
    packet.setUtf8String(this.basePath_ + path);
   ...
   * Create a symlink.
   *
   * Note: The SFTPv3 protocol says the order should be linkpath then
   * targetpath,
   ...
   * @return {!Promise<!DiskFreePacket>} A Promise that resolves or rejects
   *     with a StatusError.
   */
  statvfs(path) {
    ...
    return this.sendRequest_('statvfs@openssh.com', packet)
      .then((response) => {
        return this.isExpectedResponse_(
          response, ExtendedReplyPacket, 'STATVFS');
      })
