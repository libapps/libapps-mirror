 ...
 */
export class RelayCorpv4WsStream extends Stream {
  constructor() {
    super();
   ...
   */
  async open_(settings, resolve, reject) {
   ...
   */
  reconnect_() {
    if (!this.resume_ || ++this.reconnectCount_ > RECONNECT_LIMIT ||
        !this.sid_) {
  ...
  onSocketOpen_(e) {
    ...
    //
    // We only do this in the initial connect state as we need to resync acks
    // when we reconnect.
  ...
  onSocketData_(e) {
    ...
    switch (packet.tag) {
      ...
      case PacketTag.DATA: {
        // This creates a copy of the ArrayBuffer, but there doesn't seem to be
        // an alternative -- PPAPI doesn't accept views like Uint8Array.  If it
        // did, it would probably still serialize the entire underlying
        // ArrayBuffer (which in this case wouldn't be a big deal as it's only
        // 4 extra bytes).
  ...
  /**
   * Append ack time to ack times circular array. If array is full, find and
   * send average.
   *
